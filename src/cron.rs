//! Persistent cron job store with runtime scheduler.
//!
//! Supports three schedule types:
//! - `At(ms)` — one-time, fires at absolute timestamp, auto-deleted after execution
//! - `Every(ms)` — recurring, fires every N milliseconds
//! - `Cron(expr)` — cron expression (via `cron` crate)
...
pub enum Schedule {
    /// One-time: absolute timestamp in milliseconds.
    At(i64),
...
pub struct Job {
    ...
    pub message: String,
    pub enabled: bool,
    /// If true, send message directly to channel. If false, route through agent.
    #[serde(default = "default_true")]
    pub deliver: bool,
    /// Delete job after first execution (auto-set for At schedules).
    #[serde(default)]
...
impl CronService {
    ...
    /// Take due jobs: returns jobs whose next_run_at_ms <= now, clears their next_run.
    pub fn take_due_jobs(&mut self) -> Vec<Job> {
        ...
        for job in &mut self.jobs {
            ...
            {
                due.push(job.clone());
                job.next_run_at_ms = None; // Prevent double-fire
            }
        }
        if !due.is_empty() {
            let _ = self.save();
    ...
    /// Mark a job as executed, recompute next run or remove if one-time.
    pub fn mark_executed(&mut self, job_id: &str, success: bool) {
        ...
        if should_delete {
            ...
            if success {
                ...
                // At-type jobs that aren't deleted get disabled
...
// ── CronRunner (runtime loop) ───────────────────────────────────────────

/// Background cron runner that checks for due jobs every second.
        ...
        let handle = tokio::spawn(async move {
            ...
            while running.load(Ordering::Relaxed) {
                ...
                for job in due_jobs {
                    ...
                    if job.deliver {
                        // Direct delivery: send message to channel
                        ...
                        // Agent processing: publish as inbound so agent picks it up
...
mod tests {
    ...
    fn at_schedule_fires_and_deletes() {
        ...
        // Schedule 1ms in the past → should fire immediately
    ...
    fn every_schedule_recomputes_next_run() {
        ...
        // Manually set next_run to now to trigger
        service.jobs[0].next_run_at_ms = Some(now_ms() - 100);
        let due = service.take_due_jobs();
        assert_eq!(due.len(), 1);

        service.mark_executed(&job.id, true);
        // Should still exist with new next_run
